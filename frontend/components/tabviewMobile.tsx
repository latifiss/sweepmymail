...
export default function BottomIconTabs() { const pathname = usePathname(); return <div className="tabs__bottom">{tabs.map((tab) => <Link key={tab.id} href={tab.href} className={`tabs__tab ${pathname === tab.href || pathname.startsWith(`${tab.href}/`) ? "tabs__tab--active" : ""}`}><Image src={tab.icon} alt={tab.label} width={20} height={20} className="tabs__tab__icon" /><span className="tabs__tab__label font-gliker">{tab.label}</span></Link>)}</div>; }
